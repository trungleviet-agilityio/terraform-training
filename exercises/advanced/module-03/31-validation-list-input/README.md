...
2. Next, add a validation block to ensure that only `t3.micro` instances are used. The `condition` attribute uses the `alltrue()` function to check if all instances in the `ec2_instance_config_list` are of type `t3.micro`. If this condition is not met, Terraform will return the error message `"Only t3.micro instances are allowed."`
