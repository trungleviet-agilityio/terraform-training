...
2. Add suitable entries to the `ec2_instance_config_map` variable in the `terraform.tfvars` file. For example, one entry could be for a `ubuntu_1` instance with `t3.micro` as the `instance_type` and `ubuntu` as the `ami`.
