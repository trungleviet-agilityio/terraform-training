...
2. Add an entry for the variable in the `terraform.tfvars` file. The list should contain one object with the `instance_type` set to `"t3.micro"` (or whatever type falls under your free tier), and the `ami` set to `"ubuntu"`. Also make sure to set the `ec2_instance_count` variable to `0` so that we do not create any additional ec2 instances.
