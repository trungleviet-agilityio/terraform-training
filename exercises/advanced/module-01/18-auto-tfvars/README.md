...
1. Create a `prod.auto.tfvars` file with specific configurations for production environment.
2. Understand how Terraform automatically loads values from `*.auto.tfvars` files.
3. Observe how values in `*.auto.tfvars` files override those in `terraform.tfvars` files.
4. Test the behavior by running `terraform plan` and observing which values are used.
