...
1. Ensure that locals are defined according to previous exercises.
2. Set up an S3 bucket correctly with a `random_id` for the suffix.
3. Create an output named `s3_bucket_name` to output the bucket name with a suitable description.
4. Run and confirm a terraform apply command, and inspect what is output in the terminal.
5. Learn how to retrieve the output value outside terraform using `terraform output` commands.
