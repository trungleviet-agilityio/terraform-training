...
1. Set the `s3_bucket_name` output as sensitive by adding `sensitive = true` to the output block.
2. Retrieve the value of the sensitive output using `terraform output` commands.
3. Create a variable called `my_sensitive_value` containing a sensitive value.
4. Create an output named `sensitive_var` that reflects the sensitive variable.
5. Use the `my_sensitive_value` to create a new tag in the `local.common_tags` object and observe the impact on Terraform logs.
