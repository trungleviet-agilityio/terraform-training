...
## Prerequisites
- Terraform CLI ≥ 1.7.0 to use the `removed` block.
- Using Terraform 1.6.x or earlier? Use the CLI fallback (`terraform state rm`) instead.
...
6. Let’s now try with the `removed` block. Add the following code to the `remove.tf` file. Comment out the S3 bucket code and run `terraform plan`.
    ...
    removed {
      ...
      lifecycle {
        destroy = true # set to false to forget without deleting
      }
    }
    ```
...
## CLI Fallback (all Terraform versions)
To forget a resource without the `removed` block, run:

```
terraform state rm aws_s3_bucket.my_new_bucket
```
This updates only the state file; the physical bucket remains.
