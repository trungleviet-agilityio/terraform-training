...
> Tip: The modern approach is to use the `-replace` flag with plan/apply instead of `taint`:
> - Replace one resource: `terraform apply -replace=aws_s3_bucket.tainted`
> - Replace dependents as well: `terraform apply -replace=aws_s3_bucket.tainted -replace=aws_s3_bucket_public_access_block.from_tainted`
> `taint`/`untaint` still work, but `-replace` is preferred for clarity and explicitness.
