...
> Tips
> - Quote indexed addresses on the CLI to avoid shell parsing issues, e.g. `'aws_instance.new_list[0]'`.
> - Prefer `moved` blocks for auditable history and reproducible refactors; keep only the meaningful ones to avoid noise.
> - When moving into/out of modules, the target address uses `module.<label>.<resource_type>.<name>`.

## Desired Outcome

Here is an overview of what the created solution should execute:
...
## Troubleshooting
- Plan shows destroy/create after a rename: you likely forgot the move. Use `terraform state mv <from> <to>` or add a `moved {}` block and re‑apply.
- “Address not found” during move: validate the source address via `terraform state list` and the destination address spelling.
